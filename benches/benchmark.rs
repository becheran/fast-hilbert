...
fn criterion_benchmark(c: &mut Criterion) {
    ...
    c.bench_function("hilbert_cpp", |b| {
        ...
        b.iter(|| {
            for x in 0..n_u16 {
                for y in 0..n_u16 {
                    hilbert_xy_to_index(black_box(x), black_box(y));
                }
            }
        })
    });
    ...
    c.bench_function("fast_hilbert_low", |b| {
        b.iter(|| {
            let _h = fast_hilbert::xy2h(black_box(xy_low.0), black_box(xy_low.1), black_box(order));
        })
    });
    c.bench_function("fast_hilbert_high", |b| {
        b.iter(|| {
            let _h = fast_hilbert::xy2h(black_box(xy_high.0), black_box(xy_high.1), black_box(order));
        })
    });
    ...
    c.bench_function("hilbert_cpp_low", |b| {
        b.iter(|| {
            hilbert_xy_to_index(xy_low.0 as u16, xy_low.1 as u16);
        })
    });
}
criterion_group!(
